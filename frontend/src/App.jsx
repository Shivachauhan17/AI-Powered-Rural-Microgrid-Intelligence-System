...
import {
  ...
  Wifi, WifiOff, Bell, ChevronUp, ChevronDown,
...
// ─── Generic fetch hook ────────────────────────────────────────────────────
function useFetch(fn, fallback, pollMs) {
  ...
  useEffect(() => {
    load()
    if (pollMs > 0) timerRef.current = setInterval(load, pollMs)
...
function useRealtime() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const wsUrl = `${proto}${window.location.host}/api/v1/dashboard/ws/realtime`
      ws = new WebSocket(wsUrl)
      ws.onopen = () => setWsConnected(true)
      ws.onmessage = (e) => { try { setReading(JSON.parse(e.data)) } catch (_) { } }
      ws.onerror = () => { setWsConnected(false); pollRef.current = setInterval(fetchStats, 5000) }
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '8px 0' }}>
      ...
      <div style={{ position: 'absolute', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <span style={{ fontSize: 20, fontWeight: 700, fontFamily: 'IBM Plex Mono,monospace', color }}>{soc}%</span>
        <span style={{ fontSize: 10, color: '#64748b' }}>Charged</span>
      </div>
    </div>
  )
}
...
function HouseCard({ house }) {
  const sat = +(house.satisfaction_pct ?? house.satisfaction ?? 0)
  const dem = +(house.demand_kw ?? house.demand ?? 0)
  const alloc = +(house.allocated_kw ?? house.allocated ?? 0)
  ...
  return (
    <div className={`house-card priority-${house.priority}`}>
      <div className="house-top">
        ...
        <div style={{ flex: 1 }}>
          <span style={{ fontSize: 12, fontWeight: 600, display: 'block' }}>{house.name || house.id}</span>
          <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.05em', color }}>{house.priority === 'critical' ? '🔴 Must Serve' : house.priority === 'high' ? '🟡 Important' : '🟢 Normal'}</span>
        </div>
...
function AlertBadge({ level }) {
  ...
  return <span className="alert-badge" style={{ background: colors[level] }}>{icons[level]} {level === 'critical' ? 'Urgent' : level === 'warning' ? 'Warning' : 'Info'}</span>
}

// ─── Main App ─────────────────────────────────────────────────────────────
export default function App() {
  const [tab, setTab] = useState('dashboard')
  const { reading, wsConnected } = useRealtime()
      ...
      <aside className="sidebar">
        ...
        <nav className="nav">
          {[
            ...
            <button key={id} className={`nav-btn ${tab === id ? 'active' : ''}`} onClick={() => setTab(id)}>
              ...
              {id === 'alerts' && alertsQ.data?.filter(a => !a.resolved && a.level !== 'info').length > 0 && (
                <span className="badge">{alertsQ.data.filter(a => !a.resolved && a.level !== 'info').length}</span>
              )}
            </button>
          ))}
        </nav>
        <div style={{ borderTop: '1px solid #1e293b', paddingTop: 12, fontSize: 11, color: '#64748b', display: 'flex', flexDirection: 'column', gap: 5 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            {wsConnected ? <Wifi size={13} color="#22c55e" /> : <WifiOff size={13} color="#f97316" />}
            <span style={{ color: wsConnected ? '#22c55e' : '#f97316' }}>{wsConnected ? 'Live Updates' : 'Auto-refresh'}</span>
          </div>
        ...
        <header className="topbar">
          <div>
            <h1 style={{ fontSize: 20, fontWeight: 700 }}>
              {{ dashboard: 'Live Energy Overview', forecast: 'Energy Predictions', optimize: 'Smart Power Distribution', houses: 'Home-by-Home Status', alerts: 'Warnings & Notifications' }[tab]}
            </h1>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            {s && <div className={`risk-badge ${s.blackout_risk}`}>
        ...
        <div className="content">
          ...
          {tab === 'dashboard' && (<>
            ...
            <div className="stats-grid">
              <StatCard icon={Sun} loading={stats.loading} label="Solar Power Now" value={s?.current_solar_kw ?? '—'} unit="kW" color="#f59e0b" sub={`Total generated today: ${s?.today_solar_kwh ?? '—'} kWh`} />
              <StatCard icon={Zap} loading={stats.loading} label="Village Power Need" value={s?.current_demand_kw?.toFixed(1) ?? '—'} unit="kW" color="#6366f1" sub={`Total consumed today: ${s?.today_demand_kwh ?? '—'} kWh`} />
              <StatCard icon={Battery} loading={stats.loading} label="Battery Charge" value={s?.battery_soc_pct ?? '—'} unit="%" color="#22c55e" sub="Full charge = 50 units stored" />
              <StatCard icon={TrendingUp} loading={stats.loading} label="Energy Saved by AI" value={s?.savings_pct ?? '—'} unit="%" color="#0ea5e9" sub="compared to no AI system" />
            </div>
            ...
            <div style={{ display: 'flex', gap: 16 }}>
                ...
                {profile.loading ? <Spinner /> : profile.error ? <ErrBox msg={profile.error} onRetry={profile.refetch} /> : (
                  <ResponsiveContainer width="100%" height={250}>
                    <AreaChart data={chartData}>
                      ...
                      <Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 8 }} labelStyle={{ color: '#94a3b8' }} itemStyle={{ color: '#e2e8f0' }} />
                      <Legend />
              ...
              <div className="chart-card" style={{ flex: 1, alignItems: 'center', display: 'flex', flexDirection: 'column' }}>
                <h3 className="chart-title">Battery Status</h3>
                {stats.loading ? <Spinner /> : <>
                  <BatteryGauge soc={s?.battery_soc_pct ?? 0} />
                  <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
                    {[['Current Shortage', `${s?.deficit_kw ?? 0} kW`], ['Current Surplus', `${s?.surplus_kw ?? 0} kW`],
                    ['Homes Connected', s?.active_houses ?? '—'], ['Blackout Risk', s?.blackout_risk ?? '—']].map(([k, v]) => (
          ...
          {tab === 'forecast' && (<>
            <div className="chart-card">
              <h3 className="chart-title">How Much Solar Power We'll Have Tomorrow</h3>
              <p className="chart-desc">Expected total: ${solar.data?.total_daily_kwh ?? '—'} kWh · Peak output: ${solar.data?.peak_generation_kw ?? '—'} kW around ${solar.data?.peak_hour ?? '—'}:00 · Cloud cover: {solar.data?.cloud_cover_estimate != null ? `${(solar.data.cloud_cover_estimate * 100).toFixed(0)}%` : '—'}</p>
              ...
              {forecasts.loading ? <Spinner /> : forecasts.error ? <ErrBox msg={forecasts.error} onRetry={forecasts.refetch} /> : (
                <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={forecasts.data.map(h => ({
                    ...
                    daily_kwh: h.total_daily_kwh, peak_kw: h.peak_demand_kw, priority: h.priority
                  }))}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                    ...
                    <Bar dataKey="daily_kwh" name="Daily kWh" radius={[4, 4, 0, 0]}>
                      {forecasts.data.map((h, i) => <Cell key={i} fill={h.priority === 'critical' ? '#ef4444' : h.priority === 'high' ? '#f97316' : '#22c55e'} fillOpacity={0.8} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              )}
              ...
              {metricsQ.loading ? <Spinner /> : metricsQ.error ? <ErrBox msg={metricsQ.error} onRetry={metricsQ.refetch} /> : (
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead><tr>{['Home / Location', 'Avg Error (kW)', 'Worst Error (kW)', 'Error %', 'AI Model Used'].map(c => (
          ...
          {tab === 'optimize' && (<>
            ...
            <div className="opt-header">
              <div>
                ...
                {optim.data && <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 6 }}>
              ...
              {optim.data && <div style={{ padding: '8px 16px', borderRadius: 8, fontSize: 12, fontWeight: 600, ...(optim.data.blackout_risk === 'low' ? { background: '#22c55e18', color: '#22c55e', border: '1px solid #22c55e30' } : { background: '#f9731618', color: '#f97316', border: '1px solid #f9731630' }) }}>
                {optim.data.blackout_risk === 'low' ? '✅ Everyone Served' : '⚠️ Not Enough Power'}
              </div>}
            </div>

            {optim.data?.recommendation && <div style={{ background: '#0ea5e910', border: '1px solid #0ea5e930', borderRadius: 10, padding: '12px 16px', fontSize: 13, color: '#0ea5e9' }}>{optim.data.recommendation}</div>}

            {optim.loading ? <Spinner /> : optim.data && (<>
              <div className="alloc-table">
                <div style={{ minWidth: 700 }}>
                  <div className="alloc-header">
                    <span>Home / Location</span><span>Importance</span><span>Power Needed</span><span>Power Given</span><span>Supply Level</span><span>Status</span>
                  </div>
                  {optim.data.allocations.map(a => (
                    <div key={a.consumer_id} className="alloc-row">
                      <span style={{ fontWeight: 500 }}>{({ clinic: '🏥', school: '🏫', pump: '💧' })[a.consumer_id] || '🏠'} {a.consumer_id}</span>
                      <span className={`priority-tag ${a.priority}`}>
                        {a.priority === 'critical' ? '🔴 Must Serve' : a.priority === 'high' ? '🟡 Important' : '🟢 Normal'}
                      </span>
                      ...
                      <span className={`status-tag ${a.status}`}>
                        {a.status === 'full' ? 'Fully Powered' : a.status === 'partial' ? 'Partial Power' : 'No Power'}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <div className="chart-card">
                ...
                <ResponsiveContainer width="100%" height={230}>
                  <BarChart data={optim.data.allocations.map(a => ({ name: a.consumer_id.replace('house_', 'H').replace('clinic', 'Clinic').replace('school', 'School').replace('pump', 'Pump'), demand: a.demanded_kw, allocated: a.allocated_kw }))}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                    <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 10 }} angle={-30} textAnchor="end" height={60} />
                    <YAxis tick={{ fill: '#64748b', fontSize: 11 }} unit=" kW" />
                    <Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 8 }} />
                    <Legend />
                    <Bar dataKey="demand" fill="#6366f1" name="Demanded" radius={[3, 3, 0, 0]} fillOpacity={0.5} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              {alertsQ.loading ? <Spinner /> : (<>
                <div style={{ display: 'flex', gap: 12 }}>
                  {['critical', 'warning', 'info'].map(lvl => (
                    <span key={lvl} className={`al-count ${lvl}`}>
                      {lvl === 'critical' ? '🚨' : lvl === 'warning' ? '⚠️' : '💡'} {alertsQ.data.filter(a => a.level === lvl).length} {lvl}
                    </span>
                  ))}
                </div>
                {alertsQ.data.length === 0 && <div style={{ textAlign: 'center', color: '#64748b', padding: 40 }}>✅ All systems working normally — no warnings at this time</div>}
                {alertsQ.data.map(a => (
                  <div key={a.id} className={`alert-item ${a.level} ${a.resolved ? 'resolved' : ''}`}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1 }}>
                      <AlertBadge level={a.level} />
                      <span style={{ fontSize: 13 }}>{a.message}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, whiteSpace: 'nowrap' }}>
                      {a.resolved && <span style={{ fontSize: 11, color: '#22c55e', background: '#22c55e18', padding: '2px 8px', borderRadius: 999 }}>✓ Resolved</span>}
                ...
                <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 12, padding: 20 }}>
                  <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 8 }}>📱 SMS Notifications (Twilio)</h3>
                  <p style={{ fontSize: 12, color: '#64748b' }}>
                    Set <code style={{ background: '#f59e0b10', color: '#f59e0b', padding: '1px 5px', borderRadius: 4, fontFamily: 'IBM Plex Mono,monospace' }}>TWILIO_ACCOUNT_SID</code>,{' '}
                    <code style={{ background: '#f59e0b10', color: '#f59e0b', padding: '1px 5px', borderRadius: 4, fontFamily: 'IBM Plex Mono,monospace' }}>TWILIO_AUTH_TOKEN</code> in <code style={{ background: '#f59e0b10', color: '#f59e0b', padding: '1px 5px', borderRadius: 4, fontFamily: 'IBM Plex Mono,monospace' }}>.env</code> to enable operator SMS alerts in local language.
                  </p>
                  <p style={{ fontSize: 11, color: '#64748b', marginTop: 8, fontFamily: 'IBM Plex Mono,monospace' }}>Test: POST /api/v1/alerts/sms/test?phone=+91XXXXXXXXXX</p>
                </div>
              </>)}
            </div>
          )}
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .alloc-table { background:var(--surface); border:1px solid var(--border); border-radius:12px; overflow-x:auto; }
  .alloc-header { display:grid; grid-template-columns:1.8fr 1fr 1fr 1fr 1.5fr 1fr; gap:12px; padding:12px 20px; background:var(--surface2); font-size:11px; font-weight:600; text-transform:uppercase; letter-spacing:.08em; color:var(--muted); }
  .alloc-row { display:grid; grid-template-columns:1.8fr 1fr 1fr 1fr 1.5fr 1fr; gap:12px; padding:12px 20px; border-top:1px solid var(--border); font-size:13px; align-items:center; font-family:'IBM Plex Mono',monospace; }
  .alloc-row:hover { background:var(--surface2); }
  .priority-tag { font-size:10px; padding:2px 8px; border-radius:999px; font-weight:700; text-transform:uppercase; font-family:'Plus Jakarta Sans',sans-serif; }
  ...
  .status-tag { font-size:10px; padding:2px 8px; border-radius:999px; font-weight:600; font-family:'Plus Jakarta Sans',sans-serif; }
  .status-tag.full         { background:#22c55e20; color:#22c55e; }
  .status-tag.partial      { background:#eab30820; color:#eab308; }
  .status-tag.low,.status-tag.critical-low { background:#ef444420; color:#ef4444; }
  ...
  .alert-badge { font-size:10px; padding:3px 10px; border-radius:999px; color:white; font-weight:600; text-transform:uppercase; white-space:nowrap; }
